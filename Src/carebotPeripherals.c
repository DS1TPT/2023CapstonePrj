...
int periph_irSnsrChk(int mode) {
	halStat = HAL_ADC_PollForConversion(IR_SNSR_HANDLE, IR_SNSR_POLL_TIMEOUT);
	if (halStat != HAL_OK) // couldn't poll
		return IR_SNSR_ERR;
	adcDta8 = HAL_ADC_GetValue(IR_SNSR_HANDLE); // get data
	...
	*/
	...
	case IR_SNSR_MODE_OP:
		if (distCM <= IR_SNSR_DIST_OP) return IR_SNSR_NEAR;
		else return IR_SNSR_FAR;
		break;
	case IR_SNSR_MODE_FIND:
		if (distCM <= IR_SNSR_DIST_FIND) return IR_SNSR_NEAR;
		else return IR_SNSR_FAR;
		break;
	case IR_SNSR_MODE_SNACK:
		if (distCM <= IR_SNSR_DIST_SNACK) return IR_SNSR_NEAR;
		else return IR_SNSR_FAR;
		break;
	}
}
